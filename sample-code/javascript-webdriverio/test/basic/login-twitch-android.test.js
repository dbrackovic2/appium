...
describe("Login to gawkbox app with twitch", function () {
  let client;
  let cntxts;
  before(function () {
  ...
  it("Should choose twitch", function () {
    client.click('android=new UiSelector().text("Login").className("android.widget.TextView")');
    return client.waitForVisible('android=new UiSelector().description("twitch login button").className("android.view.ViewGroup")', 60000);
  });

  it("should open twitch webpage", function () {
    client.click('android=new UiSelector().description("twitch login button").className("android.view.ViewGroup")');
